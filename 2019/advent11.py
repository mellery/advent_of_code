...
def execute_turn(result, facing):
    if result == 0: #left 90
        turn = "left"
        if facing == "up":
            facing = "left"
...
def day11(cmd):
    ...
    for x in range(1,100000):
        ...
        grid[old_loc] = results[0]

        if new_loc not in grid:
            grid[new_loc] = 0

        ProgramA.add_input(grid[new_loc])
...
def day11p2():
    ...
    for g in grid:
        temp = list(g)
        if grid[g] == 1:
            Matrix[abs(temp[0])][abs(temp[1])] = '*'#grid[g]
