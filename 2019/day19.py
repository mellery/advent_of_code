...
def checkPos(x,y):
    ...
    Program.start()
    while Program.halted == False and Program.needInput == False:
        wait = 0
    Program.add_input(x)
    while Program.halted == False and Program.needInput == False:
        wait = 0
...
def day19p1():
    ...
    for x in range(0,gridsize):
        for y in range(0,gridsize):
            sum = sum + checkPos(x,y)
    return sum

def day19p2():
    shipsize = 100-1
    x = y = 0
    while not checkPos(x+shipsize, y):
        y += 1
        while not checkPos(x, y+shipsize):
            x += 1
    return x * 10000 + y
